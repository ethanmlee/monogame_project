...
*/
    ...
    public class PerlinNoise
    {
        ...
        public PerlinNoise(int seed, int? periodSize = null)
        {
        ...
        private int Wrap(int v, double period)
        {
            if (v < 0)
                return (int)Math.Floor((v % period) + period);
            return (int)Math.Floor(v % period);
        }
        
        public double Noise(double x, double y, double z, double scale = 1.0)
        {
            ...
            int ix = Wrap((int)Math.Floor(x), (_periodSize ?? int.MaxValue) * scale);
            double fx0 = x - ix;
            double fx1 = fx0 - 1;
            double wx = Smooth(fx0);

            int iy = Wrap((int)Math.Floor(y), (_periodSize ?? int.MaxValue) * scale);
            double fy0 = y - iy;
            double fy1 = fy0 - 1;
            double wy = Smooth(fy0);

            int iz = Wrap((int)Math.Floor(z), (_periodSize ?? int.MaxValue) * scale);
            double fz0 = z - iz;
            double fz1 = fz0 - 1;
